...
# print("")
# print("wireOnePoints")
for currentMove in wireOne:
    if currentMove[0] == "U":
        currentY += int(currentMove[1:])
    if currentMove[0] == "D":
        currentY -= int(currentMove[1:])
    if currentMove[0] == "R":
        currentX += int(currentMove[1:])
    if currentMove[0] == "L":
        currentX -= int(currentMove[1:])

    newPoint = Point(currentX, currentY)
    wireOnePoints.append(newPoint)

    if newPoint.tupleValue() in wireOneStepDict:
        steps = wireOneStepDict[newPoint.tupleValue()]
    else:
        steps += int(currentMove[1:])

    wireOneStepDict[newPoint.tupleValue()] = steps
    # print("(" + str(currentX) + ", " + str(currentY) + ")")

# print(wireOneStepDict)
...
for currentMove in wireTwo:
    ...
    newPoint = Point(currentX, currentY)
    wireTwoPoints.append(newPoint)

    if newPoint.tupleValue() in wireTwoStepDict:
        steps = wireTwoStepDict[newPoint.tupleValue()]
    else:
        steps += int(currentMove[1:])

    wireTwoStepDict[newPoint.tupleValue()] = steps
    # print("(" + str(currentX) + ", " + str(currentY) + ")")

# print(wireTwoStepDict)

for index2 in range(len(wireTwoPoints) - 1):
    point2A = wireTwoPoints[index2]
    point2B = wireTwoPoints[index2 + 1]

    horizontalLine = True
    if point2A.x == point2B.x:
        horizontalLine = False

    for index1 in range(len(wireOnePoints) - 1):
        point1A = wireOnePoints[index1]
        point1B = wireOnePoints[index1 + 1]

        # print("")
        # print("Point 2A: (" + str(point2A.x) + ", " + str(point2A.y) + ")")
        # print("Point 2B: (" + str(point2B.x) + ", " + str(point2B.y) + ")")
        # print(horizontalLine)
        # print("Point 1A: (" + str(point1A.x) + ", " + str(point1A.y) + ")")
        # print("Point 1B: (" + str(point1B.x) + ", " + str(point1B.y) + ")")

        if horizontalLine == True:
            if point1A.y == point1B.y:
                continue

            if (point1A.y <= point2A.y <= point1B.y or point1B.y <= point2A.y <= point1A.y) and (point2A.x <= point1A.x <= point2B.x or point2B.x <= point1A.x <= point2A.x):
                iPoint = Point(point1A.x, point2A.y)
                intersectionPoints.append(iPoint)

                if iPoint.tupleValue not in intersectionSteps1:
                    intersectionSteps1[iPoint.tupleValue(
                    )] = wireOneStepDict[point1A.tupleValue()] + abs(point1A.y - iPoint.y)

                if iPoint.tupleValue not in intersectionSteps2:
                    intersectionSteps2[iPoint.tupleValue(
                    )] = wireTwoStepDict[point2A.tupleValue()] + abs(point2A.x - iPoint.x)

                # print("iPoint: (" + str(point1A.x) + ", " + str(point2A.y) + ")")
            else:
                continue
            ...
            if (point1A.x <= point2A.x <= point1B.x or point1B.x <= point2A.x <= point1A.x) and (point2A.y <= point1A.y <= point2B.y or point2B.y <= point1A.y <= point2A.y):
                iPoint = Point(point2A.x, point1A.y)
                intersectionPoints.append(iPoint)

                if iPoint.tupleValue not in intersectionSteps1:
                    intersectionSteps1[iPoint.tupleValue(
                    )] = wireOneStepDict[point1A.tupleValue()] + abs(point1A.x - iPoint.x)

                if iPoint.tupleValue not in intersectionSteps2:
                    intersectionSteps2[iPoint.tupleValue(
                    )] = wireTwoStepDict[point2A.tupleValue()] + abs(point2A.y - iPoint.y)

                # print("iPoint: (" + str(point2A.x) + ", " + str(point1A.y) + ")")
            else:
                continue
...
minDistance = 0
minSteps = 0
minStepsManhattanDistance = 0

for currentPoint in intersectionPoints:
    manhattanDistance = currentPoint.mDis()
    steps1 = intersectionSteps1[currentPoint.tupleValue()]
    steps2 = intersectionSteps2[currentPoint.tupleValue()]
    totalSteps = steps1 + steps2
    print("(" + str(currentPoint.x) + ", " + str(currentPoint.y) + ")")
    print("Manhattan Distance: " + str(manhattanDistance))
    print("Steps: " + str(steps1) + " + " +
          str(steps2) + " = " + str(totalSteps))
    if minDistance == 0 or minDistance > manhattanDistance:
        minDistance = manhattanDistance
    if minSteps == 0 or minSteps > totalSteps:
        minSteps = totalSteps
        minStepsManhattanDistance = manhattanDistance

print("Minimum Distance: " + str(minDistance))
print("Minimum Steps: " + str(minSteps))
print("Minimum Steps Distance: " + str(minStepsManhattanDistance))
